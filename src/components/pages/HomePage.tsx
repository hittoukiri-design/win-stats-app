          ...
          <div className="min-h-[300px]">
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                {/* Additional Container - Connected to CMS */}
                {bonuses.length > 0 && (
                  <AnimatedElement delay={bonuses.length * 100}>
                    <BonusCard bonus={bonuses[bonuses.length - 1]} />
                  </AnimatedElement>
                )}
                {/* View All Bonuses Container */}
                {bonuses.length > 0 && (
                  <AnimatedElement delay={(bonuses.length + 1) * 100}>
                    <Link to="/bonuses" className="no-underline">
                      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 border border-slate-700/50 p-8 h-full flex flex-col hover:border-primary/60 transition-all duration-300 group cursor-pointer">
                        ...
                        <div className="relative z-10 flex flex-col h-full items-center justify-center text-center">
                          ...
                          <div className="space-y-3 mb-8 flex-grow">
                            <div className="text-sm text-gray-400 font-paragraph leading-relaxed">
                              Discover all available bonuses and promotions
                            </div>
                            <div className="text-xs text-gray-500 font-paragraph leading-relaxed opacity-80">
                              Updated regularly with new offers
                            </div>
                          </div>

                          {/* Arrow Button */}
                          <button className="w-full py-3 px-6 border-2 border-primary text-primary font-heading font-bold rounded-xl hover:bg-primary hover:text-white transition-all duration-300 text-center text-base hover:shadow-lg hover:shadow-primary/50 group-hover:translate-x-1">
                            View All →
                          </button>
                        </div>
                      </div>
